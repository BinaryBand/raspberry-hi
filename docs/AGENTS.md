...
| systemd | `minio`, `baikal` app roles | Hard assert — role aborts immediately |
| Linux (`/proc`, `/sys` paths) | `storage` role, storage scripts | Implicit — no guard, will fail at runtime |

**Non-systemd targets (OpenRC, runit):** Podman quadlets, `loginctl enable-linger`, and `systemd scope: user` are all systemd-specific. There is no quadlet equivalent for OpenRC/runit. The `minio` and `baikal` roles will assert and abort on non-systemd hosts rather than failing mid-play. For OpenRC targets, Podman containers would need to be managed via `rc-service` with custom init scripts generated by `podman generate systemd --new` (repurposed as a template).
