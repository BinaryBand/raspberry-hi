...
def decrypt_vault() -> dict:
    """Decrypt the vault file and return its contents as a dict."""
...
def encrypt_vault(secrets: dict) -> None:
    """Write secrets dict to an encrypted vault file."""
    plaintext = yaml.dump(secrets, default_flow_style=False)

    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".yml", delete=False, dir=ANSIBLE_DIR
    ) as tmp:
        tmp.write(plaintext)
    ...
    try:
        result = subprocess.run(
            [
                "ansible-vault", "encrypt", str(tmp_path),
                "--vault-password-file", str(VAULT_PASSWORD_FILE),
...
def prompt_missing(secrets: dict) -> dict:
    """Prompt for any secrets not already present. Returns only the new entries."""
    missing = [s for s in SECRETS if not secrets.get(s["key"])]
    if not missing:
        return {}
...
def main() -> None:
    ...
    existing = decrypt_vault() if VAULT_FILE.exists() else {}
